...
from actorch.algorithms.algorithm import Algorithm, DistributedDataParallelAlgorithm, RefOrFutureRef, Tunable
...
class REINFORCE(Algorithm):
    ...
    def setup(self, config: "Dict[str, Any]") -> "None":
        ...
        self._policy_network_optimizer_lr_scheduler = self._build_policy_network_optimizer_lr_scheduler()
    ...
    def _checkpoint(self) -> "Dict[str, Any]":
        ...
        checkpoint["policy_network_optimizer"] = self._policy_network_optimizer.state_dict()
        if self._policy_network_optimizer_lr_scheduler is not None:
            checkpoint["policy_network_optimizer_lr_scheduler"] = self._policy_network_optimizer_lr_scheduler.state_dict()
    ...
    def _checkpoint(self, value: "Dict[str, Any]") -> "None":
        ...
        self._policy_network_optimizer.load_state_dict(value["policy_network_optimizer"])
        if "policy_network_optimizer_lr_scheduler" in value:
            self._policy_network_optimizer_lr_scheduler.load_state_dict(value["policy_network_optimizer_lr_scheduler"])
    ...
    def _build_buffer_dataset(self) -> "BufferDataset":
        if self.buffer_dataset_config is None:
            batch_size = self.train_num_episodes
    ...
    def _build_policy_network_optimizer_lr_scheduler(self) -> "Optional[lr_scheduler._LRScheduler]":
    ...
    def _train_on_batch_policy_network(
        ...
        if entropy_coeff < 0.0:
            raise ValueError(f"`entropy_coeff` ({entropy_coeff}) must be in the interval [0, inf)")
        with (
