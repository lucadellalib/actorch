...
from actorch.algorithms.reinforce import DistributedDataParallelREINFORCE, REINFORCE
...
from actorch.networks import DistributionParametrization, Network, NormalizingFlow, Processor
...
class A2C(REINFORCE):
    ...
    def setup(self, config: "Dict[str, Any]") -> "None":
        ...
        super().setup(config)
        self._value_network = self._build_value_network().train().to(self._device, non_blocking=True)
        self._value_network_loss = self._build_value_network_loss()
        self._value_network_optimizer = self._build_value_network_optimizer()
        self._value_network_optimizer_lr_scheduler = self._build_value_network_optimizer_lr_scheduler()
    ...
    def _checkpoint(self) -> "Dict[str, Any]":
        ...
        checkpoint["value_network_optimizer"] = self._value_network_optimizer.state_dict()
        if self._value_network_optimizer_lr_scheduler is not None:
            checkpoint["value_network_optimizer_lr_scheduler"] = self._value_network_optimizer_lr_scheduler.state_dict()
    ...
    def _checkpoint(self, value: "Dict[str, Any]") -> "None":
        ...
        if "value_network_optimizer_lr_scheduler" in value:
            self._value_network_optimizer_lr_scheduler.load_state_dict(value["value_network_optimizer_lr_scheduler"])
    ...
    def _build_value_network(self) -> "Network":
        ...
        unexpected_keys = self.value_network_preprocessors.keys() - unnested_single_observation_space.keys()
        if not unexpected_keys:
            # Order matters
            self.value_network_preprocessors = {
                key: self.value_network_preprocessors.get(
                    key, self._get_default_value_network_preprocessor(space),
                )
        ...
        if getattr(self, "value_network_distribution_builders", None) is None:
            self.value_network_distribution_builders = {
                "value": Deterministic,
            }
            if getattr(self, "value_network_distribution_parametrizations", None) is None:
            ...
            if getattr(self, "value_network_distribution_configs", None) is None:
        ...
        if getattr(self, "value_network_normalizing_flows", None) is None:
    ...
    def _build_value_network_optimizer_lr_scheduler(self) -> "Optional[lr_scheduler._LRScheduler]":
    ...
    def _train_on_batch(
        ...
        with (
            ...
            if self.normalize_advantage:
                ...
                advantages *= mask
                advantages_stddev = (
                    ((advantages ** 2).sum(dim=1, keepdim=True) / length).sqrt().clamp(min=1e-6)
                )
        ...
        result["value_network"] = self._train_on_batch_value_network(
            state_values, targets, is_weight, mask,
        )
        result["policy_network"] = self._train_on_batch_policy_network(
            experiences, advantages, mask,
        )
    ...
    def _optimize_value_network(self, loss: "Tensor") -> "Dict[str, Any]":
        ...
        grad_l2_norm = clip_grad_norm_(self._value_network.parameters(), max_grad_l2_norm)
    ...
    def _get_default_value_network_preprocessor(self, space: "spaces.Space") -> "Processor":
    ...
    def get_worker_cls(cls) -> "Type[Trainable]":
        class Worker(super().get_worker_cls()):
            ...
            def setup(self, config: "Dict[str, Any]") -> "None":
                ...
                self._value_network_normalizing_flows = self._value_network.normalizing_flows
                prepare_model_kwargs = {}
                if not any(x.requires_grad for x in self._value_network_wrapped_model.parameters()):
